...
**Target:** Ubuntu Server Minimal (netplan + systemd)

**Purpose:** Explain how to use the setup\_system.sh script (interactive and unattended), detail what each step does and why, list required packages, describe environment variables and CLI flags, and explain design philosophy and security considerations.
...
Run with `sudo bash setup.sh` for interactive mode, or use --unattended and environment variables/flags for automated provisioning.
...
```
sudo UNATTENDED=1 \
       SET_ROOT_PASS=y ROOT_PASS_MODE=r ROOT_PASS_LEN=14 \
       CREATE_USER=y NEWUSER=admin USER_PASS_MODE=r USER_PASS_LEN=12 \
       HOSTNAME_NEW=server01 \
       SET_STATIC=n \
       bash setup_system.sh --unattended   
```
...
*   `--hostname=<name>` — set HOSTNAME_NEW directly
    
*   `--user=<username>` — set NEWUSER directly
    
*   `--user-pass=<password>` — set USER_PASS directly (less secure, visible from process list)
    
*   `--root-pass=<password>` — set ROOT_PASS_MANUAL directly (less secure)
    
*   `--static-ip` — equivalent to SET_STATIC=y (requires additional env vars to fully configure)
...
*   `SET_ROOT_PASS` — y/n — whether to set a new root password (interactive default: prompt; unattended default: n if not set)
    
*   `ROOT_PASS_MODE` — r (random) / t (typed). Default: r.
    
*   `ROOT_PASS_LEN` — integer length for randomized root password (default used in script: 14; changeable by env var)
    
*   `ROOT_PASS_MANUAL` — explicit root password string (used in unattended mode if you want to provide a typed password).
    
*   `CREATE_USER` — y/n — whether to create a new user (interactive default: prompt; unattended default: n if not set)
    
*   `NEWUSER` — username to create (e.g., admin)
    
*   `USER_PASS_MODE` — r (random) / t (typed) — default r
    
*   `USER_PASS_LEN` — length for randomized user password (default used in script: 11; changeable)
    
*   `USER_PASS` — explicit password string for the new user (in unattended mode)
    
*   `HOSTNAME_NEW` — hostname to set (defaults to current hostname)
    
*   `SET_STATIC` — y/n — whether to configure a static IPv4 address
...
*   `NETPLAN_FILE` — path where the netplan config will be written (script default: /etc/netplan/01-netcfg.yaml)
...
*   If yes, asks whether to generate a randomized password or enter one manually.
    ...
    *   If randomized: chooses a length (default 13–15 in earlier version; in the final script `ROOT_PASS_LEN` default is used), generates a secure string using openssl rand (preferred) or /dev/urandom fallback, saves it to /root/.root.pass, sets root's password using chpasswd and sets file perms to 600.
...
*   `openssl rand -base64 N | tr -dc 'A-Za-z0-9' | head -c` — for generating alphanumeric pass
    ...
    *   `tr -dc 'A-Za-z0-9' </dev/urandom` — fallback generator
    
*   `echo "root:<password>" | chpasswd` — sets root password
...
> ⚠️**Security note:** Storing passwords in /root is convenient but should be considered a temporary convenience. Best practice: rotate these passwords after initial login or move to an encrypted secret store.
...
*   Prompts for password mode: randomized or typed. If randomized, it generates a password (default length 11) and saves it to `/root/.<username>.pass`. If typed, secure input is read.
    
*   Creates the user (`adduser --disabled-password --gecos "" <username>`), sets the password via chpasswd, and adds the user to sudo group; if `usermod -aG sudo` fails it attempts `usermod -aG wheel` for distributions using wheel.
...
*   `adduser --disabled-password --gecos "" <username>` — create user without setting the password (we set it next)
...
*   If new hostname differs, writes it to `/etc/hostname`, runs `hostnamectl set-hostname <hostname>` , and restarts `systemd-logind.service` to reflect changes in the login sessions.
...
*   Optionally switches to the new user (`sudo -i -u <username>`) in interactive session when requested.
...
*   `/root/.<username>.pass` — generated user password for **<username>**.
...
*   `/etc/netplan/01-netcfg.yaml` — static IP YAML created/overwritten if `SET_STATIC` is enabled.
...
*   Interface detection fails (script can't identify interface): set `IPADDR`, `IFACE`, etc., explicitly in env vars and modify `NETPLAN_FILE` manually if necessary.
...
*   Rerun with logging: sudo bash `setup.sh 2>&1 | sudo tee /var/log/system-setup.log` and inspect the log.
...
```
sudo shred -u /root/.root.pass /root/.<username>.pass
# or
sudo rm -f /root/.root.pass /root/.<username>.pass   
```
...
```sh
sudo UNATTENDED=1 \
    SET_ROOT_PASS=y ROOT_PASS_MODE=t ROOT_PASS_MANUAL='S3cureP@ssw0rd' \
    CREATE_USER=y NEWUSER=deployer USER_PASS_MODE=t USER_PASS='DeployPass123' \
    HOSTNAME_NEW=deploy-01 \
    SET_STATIC=n \
    bash setup_system.sh --unattended   
```
...
*   **Do not** rely on this script as the sole security measure. Use it to bootstrap systems and then apply your own security baseline.
    


Disclaimer
----------
The author(s) or anyone involved with the script or the repository is not liable for any security mishaps left by the script. This script is for educational purposes only.
